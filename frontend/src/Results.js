...
export default function Results({ emlData: emlDataProp, onBack }) {
  ...
  // ── helpers to read/write sessionStorage ──
  const SS_KEY_REPORT   = "checkmail_report";
  const SS_KEY_MSGS     = "checkmail_msgs";
  const SS_KEY_HISTORY  = "checkmail_history";
  const SS_KEY_EMAILID  = "checkmail_emailId";
  ...
  // Determine if the current emlData matches the cached session
  const cachedEmlName = ssGet("checkmail_emlDataName");
  const isSameSession = emlData?.name && cachedEmlName && emlData.name === cachedEmlName;

  const [report, setReport]           = useState(() => isSameSession ? (ssGet(SS_KEY_REPORT) ?? null) : null);
  const [msgs,   setMsgs]             = useState(() => isSameSession ? (ssGet(SS_KEY_MSGS)   ?? []) : []);
  const [history, setHistory]         = useState(() => isSameSession ? (ssGet(SS_KEY_HISTORY) ?? []) : []);
  const [emailId, setEmailId]         = useState(() => isSameSession ? (ssGet(SS_KEY_EMAILID) ?? null) : null);
  const [input, setInput]             = useState("");
  const [typing, setTyping]           = useState(false);
  const [busy, setBusy]               = useState(() => isSameSession && ssGet(SS_KEY_REPORT) ? false : true);
  ...
  const setReportP = (v) => { const val = typeof v === "function" ? v(report) : v; ssSet(SS_KEY_REPORT, val); setReport(val); };
  const setMsgsP   = (v) => { setMsgs(prev => { const val = typeof v === "function" ? v(prev) : v; ssSet(SS_KEY_MSGS, val); return val; }); };
  const setHistoryP= (v) => { setHistory(prev => { const val = typeof v === "function" ? v(prev) : v; ssSet(SS_KEY_HISTORY, val); return val; }); };
  const setEmailIdP= (v) => { const val = typeof v === "function" ? v(emailId) : v; ssSet(SS_KEY_EMAILID, val); setEmailId(val); };

  // Save current eml name so we can detect same-session on reload
  useEffect(() => {
    if (emlData?.name) ssSet("checkmail_emlDataName", emlData.name);
  }, [emlData?.name]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const init = async () => {
      if (reportFromHistory) {
        ...
        if (reportFromHistory._id) {
          try {
            ...
            if (oldMsgs?.length > 0) {
              ...
              setEmailIdP(reportFromHistory._id);
              setBusy(false);
              setTyping(false);
              return;
            }
        ...
        setMsgsP([welcomeMsg]);
        setHistoryP(baseHistory);
        setEmailIdP(reportFromHistory._id || null);
        setBusy(false);
        setTyping(false);
        return;
      }
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      setTyping(false);
      setBusy(false);
      setMsgsP([{ role: "bot", text: welcomeText, chips }]);
      setHistoryP(baseContext);
  ...
  const sendMessage = useCallback(
    async (overrideText) => {
      ...
      setHistoryP((p) => [...p, { role: "user", content: text }]);
      setTyping(true);
      ...
      try {
        ...
        setTyping(false);
        setMsgsP((p) => [...p, { role: "bot", text: reply, chips: [] }]);
        setHistoryP((p) => [...p, { role: "assistant", content: reply }]);
        if (emailId) {
